...
int print_d(int input, int fd)
{
	...
	if (fd == STDERR_FILENO)
		__putchar = _eputchar; /* Write to stderr */
	if (input < 0)
	{
		_abs_ = -input;
		__putchar('-'); /* Print minus sign for negative numbers */
		count++;
	}
	else
		_abs_ = input;
...
char *convert_number(long int num, int base, int flags)
{
	...
	*ptr = '\0';
	...
	if (sign)
		*--ptr = sign; /* Append sign for negative numbers */
	return (ptr);
}
...
void remove_comments(char *buf)
{
	...
	for (i = 0; buf[i] != '\0'; i++)
		if (buf[i] == '#' && (!i || buf[i - 1] == ' '))
		{
			buf[i] = '\0'; /* Terminate the string at the '#' */
			break;
		}
}
